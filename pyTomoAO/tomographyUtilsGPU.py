...
_fast_math_kernel = cp.RawKernel(
    r"""
...
def _kv56(z_gpu, use_float32=False):
    """GPU implementation of the K_{5/6} Bessel function.

    Uses an ElementwiseKernel with dynamic parallelism.
    """
...
def _compute_block(rho_block_gpu, L0, cst, var_term, use_float32=False, zero_tol=0.0):
    ...
    if cp.any(mask_gpu):
        ...
        bessel_output = _kv56(
            cp.real(u_gpu.astype(cp.complex64 if use_float32 else cp.complex128)), use_float32
        )
...
def _auto_correlation(
    tomoParams, lgsWfsParams, atmParams, lgsAsterismParams, gridMask, use_float32=False
):
    """
    ...
    S_gpu = (
        S_tmp_gpu.reshape(nGs, nGs, mask_sum, mask_sum)
        .transpose(0, 2, 1, 3)
        .reshape(nGs * mask_sum, nGs * mask_sum)
    )

    # Make symmetric
    return cp.tril(S_gpu) + cp.triu(S_gpu.T, 1)


def _cross_correlation(
    tomoParams, lgsWfsParams, atmParams, lgsAsterismParams, gridMask=None, use_float32=False
):
    """
...
def _build_reconstructor_model(
    tomoParams, lgsWfsParams, atmParams, lgsAsterismParams, use_float32=False, alpha=1
):
    """
    ...
    with stream:
        ...
        Gamma = cp.array(block_diag([Gamma] * lgsAsterismParams.nLGS).toarray(), dtype=dtype)
        cp.get_default_memory_pool().free_all_blocks()  # Free memory after Gamma computation

        Cxx = _auto_correlation(
            tomoParams, lgsWfsParams, atmParams, lgsAsterismParams, _gridMask, use_float32
        ).astype(dtype)
        ...
        if isinstance(tomoParams.fitSrcWeight, np.ndarray):
            ...
            tomoParams.fitSrcWeight = (
                cp.ones(tomoParams.nFitSrc**2, dtype=dtype) / tomoParams.nFitSrc**2
            )
...
def _build_reconstructor_im(
    IM, tomoParams, lgsWfsParams, atmParams, lgsAsterismParams, dmParams, use_float32=False, alpha=1
):
    """
    ...
    with stream:
        ...
        Cxx = _auto_correlation(
            tomoParams, lgsWfsParams, atmParams, lgsAsterismParams, gridMask, use_float32
        ).astype(dtype)
        cp.get_default_memory_pool().free_all_blocks()  # Free memory after Cxx computation
