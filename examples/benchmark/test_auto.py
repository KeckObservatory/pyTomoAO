    ...
    if max(n, m) > block_size:
        ...
        for i in range(0, n, block_size):
            ...
            for j in range(0, m, block_size):
                ...
                out[i:i_end, j:j_end] = compute_block(block, L0, cst, var_term)
...
def calculate_scaled_shifted_coords(
...
def auto_correlation(tomoParams, lgsWfsParams, atmParams, lgsAsterismParams, gridMask):
    ...
    for k in range(len(kGs)):
        ...
        for kLayer in range(nLayer):
            ...
            iZ = calculate_scaled_shifted_coords(
                x1, y1, srcACdirectionVector, iGs, altitude, kLayer, srcACheight
            )
            jZ = calculate_scaled_shifted_coords(
...
def build_reconstructor(tomoParams, lgsWfsParams, atmParams, lgsAsterismParams):
    ...
    Cxx = auto_correlation(tomoParams, lgsWfsParams, atmParams, lgsAsterismParams, _gridMask)
    ...
    return d * _wavefront2Meter * RecStatSA
