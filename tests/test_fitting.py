...
class TestFitting:
    ...
    def test_extract_actuator_coordinates(self, fit_instance):
        ...
        test_map = np.array([[0, 1, 0], [1, 0, 1], [0, 1, 0]])
    ...
    def test_set_influence_function(
        self, mock_show, mock_imshow, mock_figure, fit_instance, mock_dm_params
    ):
    ...
    def test_map_actuators_to_new_grid_old(self, fit_instance):
        ...
        expected_coords = [
            (1 * 10 / 5 + 0.5, 1 * 10 / 5 + 0.5),
            (3 * 10 / 5 + 0.5, 3 * 10 / 5 + 0.5),
        ]
