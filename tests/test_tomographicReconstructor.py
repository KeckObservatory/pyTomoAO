...
def simple_config(tmp_path):
    ...
    config = {
        ...
        "lgs_asterism": {"height": 90000, "radiusAst": 7.6, "nLGS": 4, "wavelength": 5.89e-7},
        "lgs_wfs": {
            "nSubap": 20,
        },
        "tomography": {
...
def test_reconstruct_wavefront(simple_config, mock_parameter_classes):
    ...
    assert wavefront.shape == grid_mask.shape, (
        f"Expected shape {grid_mask.shape}, got {wavefront.shape}"
    )
    ...
    assert np.all(valid_values == 800), (
        f"Expected all valid values to be 800, got {np.unique(valid_values)}"
    )
...
def test_R_property_alias(simple_config, mock_parameter_classes):
    ...
    assert np.array_equal(reconstructor.reconstructor, reconstructor.R), (
        "R setter didn't update reconstructor property"
    )
...
def test_initialization_with_invalid_config():
    ...
    with pytest.raises(FileNotFoundError):
        tomographicReconstructor("nonexistent_config.yaml")
...
def test_build_reconstructor_error(simple_config, mock_parameter_classes):
    ...
    with patch.object(
        tomographicReconstructor, "build_reconstructor", side_effect=ValueError("Test error")
    ):
...
def test_full_reconstruction(kapa_config):
    ...
    assert wavefront.shape == expected_shape, (
