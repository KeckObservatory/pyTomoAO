...
def run_pytest_coverage(
    ...
    pytest_cmd.extend([f"--cov={cov_target_rel}", "--cov-report=term-missing"])
    ...
    try:
        result = subprocess.run(
            pytest_cmd, capture_output=True, text=True, env=env, cwd=repo_path, check=False
        )
...
def main():
    ...
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Show detailed output during analysis"
    )
    parser.add_argument(
        "--coverage-threshold",
        type=float,
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Print extensive debugging information"
    )
    ...
    if line_count <= args.line_count_threshold:
        print(
            f"Skipping analysis: Source directory has {line_count} lines "
            f"(threshold: {args.line_count_threshold})"
        )
        sys.exit(0)
    ...
    covered = coverage_results["statements"] - coverage_results["missing"]
    statements_str = f"{covered}/{coverage_results['statements']}"

    print(
        f"{args.source_dir:<20} | {line_count:<7} | {coverage_str:<25} | "
        f"{statements_str:<15} | {status}"
    )
    print("-" * 80)
